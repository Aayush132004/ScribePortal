...
const ScribeHome = () => {
  ...
  // accessibility: darker green to meet contrast
  const acceptButtonClasses = highContrast
    ? "bg-green-800 hover:bg-green-900 border-2 border-green-900 text-white"
  ...
  return (
    <div className={`min-h-screen ${baseClasses} transition-colors duration-300`}>
      ...
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-50 bg-white text-black px-3 py-2 rounded"
      >
      ...
      {/* IMPORTANT ACCESSIBILITY CHANGE:
          Use a non-banner region for this page header so we don't create a second "banner" landmark
          (Navbar likely already contains the primary banner landmark). Also ensure unique landmark names. */}
      <div role="region" aria-label="Page header" className="container mx-auto px-4 py-8">
        {/* Header Section */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-4">
            {t.studentRequests}
          </h1>
          {/* Preserve heading order by using h2 for the subtitle */}
          <h2 className={`text-lg ${highContrast ? 'text-gray-300' : 'text-gray-400'}`}>
            {t.reviewManage}
          </h2>
        </div>
        ...
        <div className="mb-8">
          <button
            ...
            className={`
              ...
              font-semibold py-3 px-6 rounded-lg shadow-lg transition-all duration-200
              disabled:opacity-50 disabled:cursor-not-allowed
              flex items-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2
            `}
            aria-label={loading ? t.loading : t.getAllStudents}
          >
            {loading ? (
      ...
      {/* main landmark */}
      <main id="main" role="main" className="container mx-auto px-4 py-8" aria-label="Student requests list">
        {/* Student Requests List */}
        {reqdata != null && reqdata.length > 0 ? (
          <div className="space-y-4" role="list" aria-label="Pending student requests">
            {reqdata.filter(request => request.isAccepted === 'wait').map((request, index) => (
              <div key={request._id || index} className="max-w-4xl mx-auto" role="listitem">
                <div className={`
                  ${cardClasses} rounded-xl p-6 shadow-xl
                  transition-all duration-200 hover:shadow-2xl
                `}>
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div className="flex items-center space-x-4 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        {/* student heading promoted to h2 to avoid skip-level */}
                        <h2 className="text-xl font-semibold mb-2 truncate" tabIndex={-1}>
                          {t.studentId}: {request.studentId}
                        </h2>
                        <div className={`text-sm space-y-1 ${highContrast ? 'text-gray-300' : 'text-gray-400'}`}>
                          <p className="flex items-center">
                    ...
                    <div className="flex-shrink-0 flex gap-3">
                      ...
                      <button
                        ...
                        className={`
                          ...
                          font-medium py-2 px-4 rounded-lg transition-all duration-200
                          disabled:opacity-50 disabled:cursor-not-allowed
                          flex items-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2
                        `}
                        aria-label={acceptingId === request._id ? t.accepting : `${t.accept} ${t.studentId} ${request.studentId}`}
                      >
                        {acceptingId === request._id ? (
                      ...
                      <button
                        ...
                        className={`
                          ...
                          font-medium py-2 px-4 rounded-lg transition-all duration-200
                          disabled:opacity-50 disabled:cursor-not-allowed
                          flex items-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2
                        `}
                        aria-label={`${t.reject} ${t.studentId} ${request.studentId}`}
                      >
                        <X className="w-4 h-4" aria-hidden="true" />
        ...
        {showRejectModal && (
          ...
          >
            <div
              ...
              className={`${modalClasses} rounded-xl shadow-2xl max-w-md w-full mx-4`}
              onClick={(e) => e.stopPropagation()}
              tabIndex={0} // make the dialog container tabbable for better keyboard focus
            >
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 id="reject-modal-title" className="text-lg font-semibold">
                  ...
                  <button
                    ...
                    className={`
                      ${highContrast ? 'text-white hover:bg-gray-800' : 'text-gray-400 hover:text-gray-300 hover:bg-gray-700'}
                      rounded-full p-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500
                    `}
                    aria-label={t.closeModal}
                  >
                    <X className="w-5 h-5" aria-hidden="true" />
